...
* How are cannabis-related businesses distributed across Minnesota?
* Is the distribution of businesses related to any economic or social indicators in the state?
...
The following steps were taken in Excel to correct data records in that list:
* Add county information to four businesses with Minnesota street addresses that had no county identification.
* In state code, three variations of MN were all coded to MN.
* One business in Tulsa, OK, was coded as Tulsa for county. No such county exists in Minnesota and it was recoded with as no county identification (out-of-state).
* A county was idnetified as "New Prauge", which is not a county. It was recoded as Le Sueur County based on its street address.
...
# Building a county-level data set for integration with data on number of cannabis-related businesses
...
I imported and selected county-level data for analysis with cannabis businesses from the [Association of Minnesota Counties](https://www.mncounties.org/information_and_jobs/county_data.php).
Specifically, I compared the number of cannabis-related businesses against population, economic, and social indicators. Data cleaning included removing asterisks and notes from the worksheets. This data was then merged with the aggregated county-level dataset of county by number of cannabis-related businesses. One missing values was for Traverse County which had no cannabis-related businesses. This was recoded from missing to 0. Another step was to merge county data from Saint Louis County and St. Louis County. Finally, no crime data was reported from Lake of the Woods County in 2021 so that remained as a missing value. 
